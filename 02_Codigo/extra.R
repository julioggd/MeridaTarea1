...
prod_agricola <- read_csv("http://infosiap.siap.gob.mx/gobmx/datosAbiertos/ProduccionAgricola/Cierre_agricola_mun_2019.csv", 
  ...
  select(Anio, Idestado, Nomestado, Idmunicipio, Nommunicipio, Nomcultivo, Valorproduccion)
...
q$Valorproduccion %>% density() %>% plot()
q$Valorproduccion %>% quantile(c(seq(0, 1, by = 0.1)))
